...
export default function Home() {
  ...
  // ── 방 삭제 ───────────────────────────────────────────────────────
  // [AUTH] deviceId를 전달해 방장만 삭제 가능. 실패 시 사용자 피드백.
    ...
    <div className="app-shell">
      <BrainHeader
        ...
        onClear={async () => {
          ...
          // [AUTH] 서버 성공 후에만 로컬 비움 + deviceId로 방장 검증
      ...
      <RoomList
        ...
        onCreateRoom={async (title: string, isPublic: boolean) => {
          // [AUTH] createdBy에 실제 deviceId 전달 — owner_device_id가 anonymous로 고정되던 버그 수정
          const res = await fetch('/api/chat', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json; charset=utf-8' },
